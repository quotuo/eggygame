...
export function LocaleDropdown({ type = 'button' }: LocaleDropdownProps) {
  ...
  return (
    <DropdownMenu>
      ...
      >
        <div className="bg-navbar grid grid-cols-2 gap-2 rounded-md">
          {Object.entries(localeNames).map(([key, name]) => (
            <DropdownMenuItem key={key} asChild>
              <Link
                className={cn(
                  'flex items-center gap-2 w-full rounded-md px-2 py-2 hover:bg-navbar-foreground/10 cursor-pointer',
                  locale === key ? 'text-navbar-foreground bg-navbar-foreground/50' : 'text-navbar-foreground/80 hover:text-navbar-foreground'
                )}
                href={cleanPathname}
                locale={key==="en"?".":key}
              >
